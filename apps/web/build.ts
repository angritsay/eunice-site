// Build the Eunice site: `node build.ts` (Node runs TypeScript directly).
...
import careers from './src/pages/careers.ts';
import company from './src/pages/company.ts';
...
// is an edit to src/content/index.ts and nothing here.
const PAGES: Page[] = [
  home,
  privateMarkets,
  digitalAssets,
...
const fonts = (asset: (path: string) => string) =>
  ['inter-latin-400-normal', 'manrope-latin-400-normal']
    .map((f) => `<link rel="preload" href="${asset(`fonts/${f}.woff2`)}" as="font" type="font/woff2" crossorigin>`)
    .join('');

// ---------- Link and asset resolution ----------
function siteCtx(page: Pick<Page, 'slug'>): Ctx {
  ...
  return {
    ...
    today: TODAY,
    preview: false,
    link(to = '', hash?: string | null, query?: string | null) {
      const q = query ? `?desk=${query}` : '';
      const h = hash ? `#${hash}` : '';
      if (to === page.slug && !q) return h || './';
      return `${up}${to ? `${to}/` : ''}${q}${h}` || './';
    },
    asset: (p: string) => `${up}assets/${p}`,
  };
}
...
function inlineAsset(p: string): string {
  if (!dataUris.has(p)) {
...
function previewCtx(page: Pick<Page, 'slug'>): Ctx {
  return {
    slug: page.slug,
    today: TODAY,
    preview: true,
...
if (SITE_ENV === 'production' && FORM_ENDPOINT && !HAS_PRIVACY_PAGE) {
  throw new Error(
    'PUBLIC_FORM_ENDPOINT is set for a production build, but there is no privacy page. See the go-live gate in docs.',
  );
}
if (SITE_ENV === 'production' && ANALYTICS_ON && !HAS_PRIVACY_PAGE) {
  throw new Error(
    'Analytics is configured for a production build, but there is no privacy page. See the go-live gate in docs.',
  );
}
...
<script src="${ctx.asset('site.js')}" defer></script>${analyticsTag()}
  ...
  fs.writeFileSync(
    ...
    `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>${nf.title}</title>${fonts((a: string) => `${config.basePath}assets/${a}`)}<link rel="stylesheet" href="${config.basePath}assets/site.css"></head><body>
...
function buildPreview() {
  ...
  const css = fs
    .readFileSync(path.join(SRC, 'assets', 'site.css'), 'utf8')
