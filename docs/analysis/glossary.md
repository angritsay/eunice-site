...
| **Placement** | Where on a page a form was opened: nav, hero, body, band, roles, closing, footer |
| **Entry point** | Page + placement (+ audience, + role). Where a lead came from; stored with it, shown to ops, used to group analytics |
| **Queue** | Who handles a submission: `leads` (business operations) or `careers` (hiring) |
| **Submission** | What a visitor sent through a form, as stored by intake |
| **Lead** | A submission in the `leads` queue |
