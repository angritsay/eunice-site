...
Dockerfile       built from the repository root; non-root, production dependencies only
```

| Service | Owns | Status |
| --- | --- | --- |
| [intake](intake/) | Form submissions (personal data, with a purge date) and the events announcing them | Implemented |
| notifier | Nothing personal: turns a submission event into an email to the ops inbox | Next |
