...
// roles script, then intake's migrations run as the schema owner.
...
export interface TestDatabase {
  ...
  /** Connects as intake_owner, as migrations do. */
  owner: Pool;
  /** Connects as intake_app, as the running service does. */
  app: Pool;
  stop(): Promise<void>;
}
...
export const runMigrations = (db: TestDatabase) => migrate({ pool: db.owner, dir: MIGRATIONS, schema: 'intake' });
